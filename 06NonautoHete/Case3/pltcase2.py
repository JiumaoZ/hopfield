...
lea = np.loadtxt('lea_case3.csv',delimiter=',')
bifa = np.loadtxt('bifa_case3.csv',delimiter=',')
lew = np.loadtxt('lew_case3.csv',delimiter=',')
bifw = np.loadtxt('bifw_case3.csv',delimiter=',')
wa = np.linspace(0,2.5,401)
...
for i in range(2):
    ...
    ax[i][0].set_xlim([0.5,2.5])
    ax[i][0].set_ylabel(yl[i],fontsize=ft)

ax[1][0].set_xlabel(r'$A_2$'+'\n (a)',fontsize=ft)
